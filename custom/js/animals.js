...
$(document).ready(function() {
	...
	manageBrandTable = $("#manageBrandTable").DataTable({
		'ajax': 'php_action/fetchAnimal.php',
		'order': []		
	});
...
function removeAnimal(animalId = null) {
	if(animalId) {
		$('#AnimalId').remove();
		$.ajax({
			url: 'php_action/fetchSelectedAnimal.php',
			type: 'post',
			data: {animalId : animalId},
			dataType: 'json',
			success:function(response) {
				$('.removeBrandFooter').after('<input type="hidden" name="AnimalId" id="AnimalId" value="'+response.animalId+'" /> ');

				// click on remove button to remove the brand
				$("#removeAnimalBtn").unbind('click').bind('click', function() {
					// button loading
					$("#removeAnimalBtn").button('loading');

					$.ajax({
						url: 'php_action/removeAnimal.php',
						type: 'post',
						data: {animalId : animalId},
						dataType: 'json',
						success:function(response) {
							console.log(response);
							// button loading
							$("#removeAnimalBtn").button('reset');
							if(response.success == true) {

								// hide the remove modal 
								$('#removeAnimalModal').modal('hide');
